...
async def execute_cand_spiders(
    ...
    Args:
        CAND_SPIDER_CREATION_RESULTS: Dictionary of candidate
            spider creation results.
    ...
    for key, chunk in CAND_SPIDER_CREATION_RESULTS.items():
        if chunk is not False:
            logger.info(
                f"Processing candidate spider",
                key=key,
                instance=actual_exec_instance
            )
            ...
            if chunk is not None and chunk.spider_code not in [None, ""]:
                ...
                if actual_exec_instance >= max_exec_instances:
                    logger.info(
                        "Reached maximum execution instances",
                        max=max_exec_instances
                    )
                    break
