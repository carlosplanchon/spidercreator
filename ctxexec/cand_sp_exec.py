...
def get_url_to_local_addresses(URL_TO_HTML: Dict[str, Dict[str, Any]]) -> Dict[str, str]:
...
async def build_fastapi_server_contexts(URL_TO_HTML: Dict[str, Dict[str, Any]]) -> List[FastAPIServerContext]:
...
async def execute_spider_with_fastapi_server_context(
    ...
    # Use AsyncExitStack to manage server_contexts in a single 'async with' block
...
class CandSpiderExecutor:
    ...
    async def start(self):
        ...
        logger.info("Extracted URLs from spider code", urls=self.runnable_spider_urls)
        ...
        logger.debug("Created URL to local address mapping", 
        ...
        logger.debug("Created FastAPI server contexts", count=len(self.server_contexts))

        # 7. Run the spider with local FastAPI servers
        self.spider_code_output_with_local_addresses: str = await execute_spider_with_fastapi_server_context(
